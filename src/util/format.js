...
export function formatAuto(
  ...
  const empty = format.replace(/%(−?)(.\d+)?([a-z])/gi, "");
...
export function formatUTC(format, date = new Date(), timezoneOffset = 0) {
  ...
  const d =
    timezoneOffset !== 0 ? new Date(+date + timezoneOffset * 60 * 1000) : date;

  return format.replace(/%(−?)(.\d+)?([a-z])/gi, (m, u, w, s) => {
    let fraction = "";
    ...
    if (w) {
      ...
      switch (s) {
        case "L":
          fraction = frac((d.getUTCFullYear() % 1000) / 1000, precision);
          break;
        case "C":
          fraction = frac((d.getUTCFullYear() % 100) / 100, precision);
          break;
        case "X":
          fraction = frac((d.getUTCFullYear() % 10) / 10, precision);
          break;
        case "Y":
          const yearDay = datetime.getYearDay(d) - 1;
          const yearLength = datetime.isLeapYear(d.getUTCFullYear())
            ? 366
            : 365;
        ...
        case "M":
          ...
          break;
        case "D":
          fraction = frac(dayFrac, precision);
          break;
        case "V":
        ...
        case "W":
          fraction = frac(
            (datetime.getWeekDay(d) - 1 + dayFrac) / 7,
            precision
          );
          break;
        case "w":
        ...
        case "O":
          ...
          break;
        case "h":
          fraction = frac((dayMs % 3600000) / 3600000, precision);
          break;
        case "m":
          fraction = frac((dayMs % 60000) / 60000, precision);
          break;
        case "s":
          fraction = frac(FRACTIONAL_SECONDS, precision);
          break;
        // We said precise, not necessarily accurate
        case "u":
          fraction = ((FRACTIONAL_SECONDS * 1e6) % 1).toFixed(precision);
          break;
        case "n":
          fraction = ((FRACTIONAL_SECONDS * 1e9) % 1).toFixed(precision);
          break;
        case "Z":
        case "z":
        default:
    ...
    switch (s) {
      case "L":
        return ((d.getUTCFullYear() / 1000) | 0).toString() + fraction;
      case "C":
        return pad2((d.getUTCFullYear() / 100) | 0) + fraction;
      case "X":
        return pad3((d.getUTCFullYear() / 10) | 0) + fraction;
      case "Y":
        return pad4(d.getUTCFullYear()) + fraction;
      case "M":
        return pad2(d.getUTCMonth() + 1) + fraction;
      case "D":
        return pad2(d.getUTCDate()) + fraction;
      case "V":
        return pad4(datetime.getWeekYear(d)) + fraction;
      case "W":
        return pad2(datetime.getWeek(d)) + fraction;
      case "w":
        return datetime.getWeekDay(d).toString() + fraction;
      case "O":
        return pad3(datetime.getYearDay(d)) + fraction;
      case "h":
        return pad2(d.getUTCHours()) + fraction;
      case "m":
        return pad2(d.getUTCMinutes()) + fraction;
      case "s":
        return pad2(d.getUTCSeconds()) + fraction;
      case "u":
        return (
          Math.round(FRACTIONAL_SECONDS * 1e6)
            .toString()
            .padStart(6, "0") + fraction
        );
      case "n":
        return (
          Math.round(FRACTIONAL_SECONDS * 1e9)
            .toString()
            .padStart(9, "0") + fraction
        );
      case "Z": {
